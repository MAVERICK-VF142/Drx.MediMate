...
if not secret_key:
    logging.warning(
        "SECRET_KEY environment variable not set. Generating a temporary random key. This should ONLY be used for local development."
    )
...
try:
    ...
    try:
        ...
        if cred_json:
            logging.info(
                "Initializing Firebase Admin SDK with credentials from FIREBASE_CREDENTIALS environment variable."
            )
            ...
            logging.info(
                f"Initializing Firebase Admin SDK with credentials at: {FIREBASE_CREDENTIALS_PATH}"
            )
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if session.get("role") != "admin":
            logging.warning(
                f"Non-admin user tried to access admin route. User role: {session.get('role')}"
            )
            return (
                render_template(
                    "unauthorized.html", message="Admin privileges required"
                ),
                403,
            )
...
def my_account():
    return render_template(
        "my_account.html",
        user={"name": "Demo User", "email": "demo@example.com", "notifications": True},
    )
...
def symptom_check():
    ...
    try:
        ...
        if action == "predict":
            ...
            logging.info(
                f"Calling get_symptom_recommendation with symptoms: {symptoms}"
            )
...
def verify_invitation():
    ...
    try:
        ...
        if not code or not email:
            return (
                jsonify({"success": False, "message": "Code and email are required"}),
                400,
            )
...
def list_invitations():
    ...
    try:
        ...
        return jsonify({"success": True, "invitations": invitations_list})
    except Exception as e:
